...
void setup() {
  ...
  pinMode(13, 1); //change out the {pin} to the pin the motor is on!
...
void loop() {
  ...
  int sens1 = analogRead(0); //replace {pin} with pin number!
  //next we get the value of sensor 2 and put it in the negatives
  int sens2 = -1 * analogRead(1); //replace {pin} with pin number!
